...
function App() {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      try { await window.ethereum.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: mainnet.chainId }] }) }
      catch (error) { if ((error as { code?: number }).code === 4902) await window.ethereum.request({ method: 'wallet_addEthereumChain', params: [mainnet] }); else throw error }
      const account = accounts[0] ?? ''
      setWallet(account)
      return account
    } catch { setToast('Connect a wallet on BOT Chain'); return '' }
  }
  ...
  return <div className="app-shell">
    ...
    <footer><a className="brand" href="#" onClick={closePrompt}><span className="brand-mark"><Sparkles size={15} /></span><span>prompt<span className="brand-accent">mint</span></span></a><span>Own the prompts that power your agents.</span><div className="footer-chain" aria-label="BOT Chain ecosystem links"><span className="footer-chain-name"><span className="chain-logo" aria-hidden="true">◈</span>BOT Chain</span><span className="footer-chain-links"><a href="https://botchain.ai" target="_blank" rel="noopener noreferrer">botchain.ai <span aria-hidden="true">↗</span></a><a href={explorerUrl} target="_blank" rel="noopener noreferrer">scan.botchain.ai <span aria-hidden="true">↗</span></a></span></div></footer>
